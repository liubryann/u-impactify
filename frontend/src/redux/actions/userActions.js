import API from '../../api';
import { USER_ERROR, USER_TYPE, SET_USER, GET_USER, UPDATE_USER, UPLOAD_USER_IMAGE, USER_COURSES } from '../types';
...
export const getAuthenticatedUserData = () => (async (dispatch) => {
  await API.get('/getAuthenticatedUser', {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem('idToken')}`
    }
  })
...
export const getUserData = (userData) => (async (dispatch) => {
  await API.get('/getUserDetails', { params: userData })
    .then((res) => {
      dispatch({
        type: GET_USER,
        payload: res.data
